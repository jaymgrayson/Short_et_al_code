...
Pop_Of_Interest<-ggplot(df2, aes(x=df2$Relapse, y=df2$`Cluster 33`)) +theme_bw()+theme(panel.border = element_blank(), panel.grid.major = element_blank(),panel.grid.minor = element_blank(), axis.line = element_line(colour = "black"))+ ggtitle("Day 0-Cluster 33") + theme(plot.title = element_text(hjust = 0.5))+geom_boxplot()
Pop_Of_Interest
