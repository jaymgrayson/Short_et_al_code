...
setwd("/Users/jasongrayson/Desktop/Critical Docs/Manuscripts/AML_Paper/JI_Version/JI_Paper_Project/Key_Data_Files/Figure_03/Stat_Runner/Day_100")
save(Normal_data,file="Normal_data_Surface.RDa")
save(Abnormal_data,file="Abnormal_data_Surface.RDa")

rm(list=ls())
load("~/Desktop/Critical Docs/Manuscripts/AML_Paper/JI_Version/JI_Paper_Project/Key_Data_Files/Figure_03/Stat_Runner/Day_100/Normal_data_Surface.RDa")
...
rm(list=ls())
load("~/Desktop/Critical Docs/Manuscripts/AML_Paper/JI_Version/JI_Paper_Project/Key_Data_Files/Figure_03/Stat_Runner/Day_100/Abnormal_data_Surface.RDa")
data_TBA<-Abnormal_data[,-c(1)]
colnames(data_TBA)[1]<-"Relapse"
data_TBA$Relapse<-as.factor(data_TBA$Relapse)
result_holder<-lapply(data_TBA[,c(2:20)],function(x)kruskal.test(x~data_TBA$Relapse))
pvalues<-rep(0,length(result_holder))
for (i in seq_along(result_holder)){
  pvalues[i]<-result_holder[[i]][[3]]
