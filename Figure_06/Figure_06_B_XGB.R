...
for (i in data_for_model)
  {
    ...
    df$class<-as.factor(df$class)
        for (j in model_runs)
        {
          ...
          data_matrix <- xgb.DMatrix(data = as.matrix(train[,-16]), label = train$class)
          params <- list(
          ...
          NB_ACC[[i,j]] <- max(xgbModel$results$Accuracy)
          test_labels<-test$class
          ROCurve<-roc(test_labels,as.numeric(xgbPredictions))
